...
export type FormValues = Omit<Consumable, "id" | "lastMovement">;
...
export const ConsumableFormModal: React.FC<Props> = ({ open, onClose, onSubmit, initial, loading }) => {
  const {
    ...
    defaultValues: initial ? { ...initial, stock: initial.stock, minStock: initial.minStock } : {},
  });
          ...
          >
            <DialogPanel className="bg-white rounded p-6 w-full max-w-md">
              ...
              <form onSubmit={handleSubmit(submit)} className="space-y-4">
                ...
                />
                {errors.minStock && (
                  <span className="text-red-600 text-sm">{errors.minStock.message}</span>
                )}
